...
m <- glm(rs~disturb_year*site, family='Gamma',data = evires)
print(summary(m), signif.stars = FALSE, digits=2)
...
anova(m)
library(multcomp)
summary(glht(m, mcp(site="Tukey")))
summary(glht(m, mcp(disturb_year="Tukey")))




anova(glm_rs)
e1 <- resid(glm_rs, type = 'pearson')
n <- nrow(evires)
p <- length(coef(glm_rs))
sum(e1^2) / (n -p)
