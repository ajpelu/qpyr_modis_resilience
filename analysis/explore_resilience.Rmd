...
# box cox 
```{r}
m <- lm(rc ~ disturb_year * site, data = evires)
...
evires$logres <- log10(evires$rs)


mod_rob 
pbad2way(rs ~ disturb_year * site, data = evires, est = "onestep")


mcp2a(rs ~ disturb_year * site, data = evires)
...
dfsel <- df %>% dplyr::select_(.dots=c(vars, resp_var)) 
myformula <- as.formula(paste0(resp_var,  " ~ ",paste(vars, collapse = '*')))
mymodel <- aov(myformula, data=dfsel)
par(mfrow=c(2,2))
plot(mymodel)
...
for (i in unique(factor(dfsel$site))){
  cat(shapiro.test(dfsel[dfsel$site==i, ]$rs)$p.value," ")
}

shapiro.test(dfsel[dfsel$site=='2005', ]$rs)

head(dfsel) 

dfsel %>% filter(site == 'Northern slope') %>% shapiro.test('rs')




bartlett.test(rs ~ disturb_year, data = dfsel)
bartlett.test(rs ~ site, data = dfsel)


factores <- c('disturb_year', 'site')
resp_var <- 'rs'
resp_var <- 'logres'
...
shapirosNormal <- function(df, resp_var, factor_vars) { 
  out <- df %>% 
    ...
    summarise(statistic = round(shapiro.test(!!resp_var)$statistic,5),
              p_value = round(shapiro.test(!!resp_var)$p.value,5))
  
  return(out)
  }


xx <-leveneTest(rs ~ site:disturb_year, data = dfsel, center=mean)

fligner.test(rs ~ site, data = dfsel)

https://rexplorations.wordpress.com/2015/09/07/two-way-anova-in-r/
http://dzchilds.github.io/aps-data-analysis-L2/
  http://www.flutterbys.com.au/stats/tut/tut7.6b.html
http://www.flutterbys.com.au/stats/tut/tut7.6a.html#h3_28
https://www.biogeosciences.net/12/4235/2015/bg-12-4235-2015.pdf
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3905046/
  https://www.researchgate.net/publication/318983016_Transpiration_and_water_potential_of_young_Quercus_petraea_M_Liebl_coppice_sprouts_and_seedlings_during_favourable_and_drought_conditions
https://canal.ugr.es/noticia/oro-esferico-tratamiento-cancer-mama/
  http://www.statmethods.net/stats/anova.html
http://www.sthda.com/english/wiki/two-way-anova-test-in-r
http://sci-hub.io/10.1111/j.1600-0706.2011.19372.x
http://rpkg.gepuro.net/
  

  
shapirosNormal(evires, resp_var = quo(logrs), factores)




out <- dfsel %>% group_by_(.dots=factores) %>% 
  summarise(statistic = shapiro.test(rs)$statistic,
            p.value = round(shapiro.test(rs)$p.value,5))
...
normal <- dfsel %>%
  mutate(log_rs = sqrt(rs)) %>% 
  group_by(disturb_year, site) %>%
  summarise(statistic = shapiro.test(log_rs)$statistic,
            p.value = round(shapiro.test(log_rs)$p.value,5))
...
glm(myformula, data=df)

head(dfsel)



leveneTest(myformula, data=dfsel)
plot(mymodel, 2)
?bartlett.test


fligner.test(root_rs ~ disturb_year*site, data=dfsel)

myformula


boxplot(root_rs ~ disturb_year, data=dfsel)



replications(root_rs ~ disturb_year*site, data=dfsel)


# Extract the residuals
aov_residuals <- residuals(object = mymodel)
